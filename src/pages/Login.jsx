    ...
    <div className="auth-container">
      <div className="auth-card">
        <div className="auth-logo">
          <img src="/logo.png" alt="Pick&Go" />
          <h1>Pick&amp;Go</h1>
          <p className="sub">{modo === 'login' ? 'Ingresá a tu cuenta' : 'Creá tu cuenta gratis'}</p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            ...
            <input className="form-input" type="password" placeholder={modo === 'registro' ? 'Mínimo 6 caracteres' : '••••••••'}
              value={password} onChange={e => setPassword(e.target.value)} required minLength={6} />
          </div>
          <button type="submit" className="btn btn-primary" style={{width:'100%',padding:'13px'}} disabled={loading}>
            {loading ? <><span className="spinner"></span> Cargando...</> : modo === 'login' ? 'Ingresar' : 'Crear cuenta'}
          </button>
        </form>
